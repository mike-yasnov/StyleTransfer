...
def image_loader(image_name, imsize, dtype):
    image = resize(imread(image_name), [imsize[0], imsize[1]])
...
class Predictor4:
    ...
    def get_image_predict(self, style_path='style.jpg', content_path='content.jpg', mode='match_style'):

        use_cuda = torch.cuda.is_available()
        dtype = torch.cuda.FloatTensor if use_cuda else torch.FloatTensor

        imsize = np.array(imread(content_path).shape[0:2]) // 2
        ...
        content_layers = ('conv_3', 'conv_4', 'conv_6', "conv_15")  # use these layers for content loss
        style_layers = (
        'conv_1', 'conv_2', 'conv_3', 'conv_4', 'conv_5', 'conv_6', 'conv_7', 'conv_8', 'conv_9', 'conv_10', 'conv_11',
        'conv_12', 'conv_13', 'conv_14', 'conv_15', 'conv_16')  # use these layers for style loss
        ...
        i = 1
        for c,layer in enumerate(list(cnn)):
